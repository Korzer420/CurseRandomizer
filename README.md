...
- Confusion (Temporarly): Switches all 8 player actions (nail, spell, dream nail etc.) with each other. Vanishes after killing a boss (Enemies with more than 200 hp are considered bosses). The needed boss kill amount is increased by 1 each time this is casted. Taking a hit has a 33% to reshuffle the controls if this curse is active. The shuffle is marked by "???" appearing.
...
- Despair (Temporarly): Prevents all other temporary curses from progressing further. Every 3 minutes you'll receive a random used curse (besides "Despair" itself). This curse vanishes after getting desperate enough (getting 7 despair point, increased by 7 each cast) and then dying. Note that dying in areas, where the shade doesn't appear do not count! Despair points can be obtained from various sources, but are capped at certain interactions. Getting cursed and killing different types of enemies grant 1 point which is uncapped though. All sources will be added to the readme... later ;)
